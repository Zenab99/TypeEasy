...
namespace type_easy
{
    public partial class Form12_type_text : Form
    {
        ...
        private void WordComboBox_init(string path)
        {
            TextComboBox.Items.Clear();
            TextComboBox.Items.AddRange(PathExt.GetAllFileInDirectory(path, ".txt").Cast<object>().ToArray());
        }
        ...
        public void Play_wav(string file, bool shouldPlay)
        {
            if (shouldPlay == true)
            {
        ...
        { //change the labels to reflect that we are at the end
        }
        ...
        {
            Properties.Settings.Default.type_article_path = TextComboBox.SelectedItem.ToString();
        ...
        private void ChangeLanguage()
        {
            ...
            {
                var resources = new ComponentResourceManager(typeof(Form12_type_text));
